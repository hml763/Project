...
dstimg = cv2.flip(cvgrayimg,1) #0 : vertical flipping around the x-axis, 0보다 크면 y-axis, 0보다 작으면 both axis

cv2.namedWindow('Horizontal Flipping')
cv2.imshow('Horizontal Flipping',dstimg)
cv2.waitKey(0)

plt.subplot(121), plt.imshow(cvgrayimg,'gray'), plt.title('Original image')
plt.xticks([]), plt.yticks([])
plt.subplot(122), plt.imshow(dstimg,'gray'), plt.title('Horizontal Flipping image')
plt.xticks([]), plt.yticks([])
